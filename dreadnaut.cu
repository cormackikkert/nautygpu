...
DYNALLSTATGPU(set,active,active_sz);
#else
static volatile __managed__ set active[MAXM];
static __managed__ graph g[MAXM*1L*MAXN];
//static graph *g; cudaMallocManaged(&g, MAXM*1L*MAXN*sizeof(graph));
static graph canong[MAXM*1L*MAXN];
static graph savedg[MAXM*1L*MAXN];
static setword workspace[MAXM*2L*WORKSIZE];
static __managed__ int lab[MAXN];
//static int *lab; cudaMallocManaged(&lab, MAXN*sizeof(int));
static __managed__ int ptn[MAXN];
//static int *ptn; cudaMallocManaged(&ptn, MAXN*sizeof(int));
...
static set tempactive[MAXM];
//static set *active; cudaMallocManaged(&active, MAXM*sizeof(set));
//static int *cnt; cudaMallocManaged(&cnt, MAXM*sizeof(int)); // scratch array
#endif
...
#if !MAXN
    n = WORDSIZE;
    DYNALLOC2GPU(graph,g,g_sz,n,m,"dreadnaut");
    DYNALLOC1GPU(int,lab,lab_sz,n,"dreadnaut");
    DYNALLOC1GPU(int,ptn,ptn_sz,n,"dreadnaut");
    DYNALLOC1(int,orbits,orbits_sz,n,"dreadnaut");
    DYNALLOC1(int,perm,perm_sz,n,"dreadnaut");
    DYNALLOC1GPU(set,active,active_sz,m,"dreadnaut");
    n = 1;
#endif
...
#if !MAXN
                    DYNALLOC1GPU(int,lab,lab_sz,newn,"dreadnaut");
                    DYNALLOC1GPU(int,ptn,ptn_sz,newn,"dreadnaut");
                    DYNALLOC1(int,orbits,orbits_sz,newn,"dreadnaut");
                    DYNALLOC1(int,perm,perm_sz,newn,"dreadnaut");
                    DYNALLOC1GPU(set,active,active_sz,newm,"dreadnaut");
#endif
                    ovalid = FALSE;
                    pvalid = FALSE;
                    freeschreier(NULL,&generators);
...
#if !MAXN
                    DYNALLOC2GPU(graph,g,g_sz,newn,newm,"dreadnaut");
#endif
